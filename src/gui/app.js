...
function renderSidebar() {
  ...
  const d = S.data;
  $('#me').innerHTML = `<b>${h(d.device_name || '')}</b><code title="Huella TLS">${h(d.fingerprint || '')}</code>${d.signer_fingerprint ? `<div class="row"><span>Firma</span><code title="Huella de firma de tickets (Ed25519): ${h(d.signer_fingerprint)}">${h(d.signer_fingerprint.split(':').slice(0,4).join(':'))}</code></div>` : ''}<div class="row"><span>LAN</span><code>${h((d.local_ips || [])[0] || '')}:${d.lan_port || ''}</code></div><div class="row"><span>${d.pin_required ? 'PIN requerido' : 'Sin PIN'}</span><span>${d.auto_accept ? 'auto-aceptar' : 'confirmar'}</span></div>${d.pairing_uri ? `<button class="btn small" id="me-pair" title="QR de emparejamiento: permite enviarte archivos sin descubrimiento mDNS">QR de emparejamiento</button>` : ''}`;
  const mp = $('#me-pair'); if (mp) mp.onclick = openPairing;
}
function openPairing() {
  const d = S.data, uri = d.pairing_uri; if (!uri) return toast('Sin dirección LAN', 'err');
  const m = modal(`<div class="m-head"><h2>Emparejar con este dispositivo</h2><button class="btn icon ghost" data-close>✕</button></div><div class="m-body"><div class="share"><div class="qr"><img src="/api/qr?data=${encodeURIComponent(uri)}" alt="QR"></div><div><h3>Ticket de emparejamiento LAN</h3><div class="linkbox"><input readonly value="${h(uri)}"><button class="btn" id="pr-cp">Copiar</button></div><p class="muted"><code>${h(d.device_name)} · ${h((d.local_ips || [])[0] || '')}:${d.lan_port} · ${h(d.fingerprint)}${d.pin_required ? ' · PIN incluido' : ''}</code></p><p class="hint">Quien escanee este QR (o pegue el ticket en «Enviar por LAN → dirección manual» / <code>send-lan --to</code>) podrá enviarte archivos sin descubrimiento mDNS, con tu huella TLS fijada y el PIN ya incluido. No lo publiques: equivale a tu PIN.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><a class="btn" href="/api/ticket/file?uri=${encodeURIComponent(uri)}" download="${h(d.device_name)}.unishare">⬇ Guardar .unishare</a><a class="btn" href="/api/qr?data=${encodeURIComponent(uri)}" download="pairing.svg">Guardar QR (SVG)</a></div></div></div></div><div class="m-foot"><button class="btn" data-close>Cerrar</button></div>`, 'wide');
  $('#pr-cp', m).onclick = () => copy(uri);
}
...
function renderDetails() {
  ...
  if (S.tab === 'general') {
    pane.innerHTML = `<div class="two"><dl class="kv"><dt>Nombre</dt><dd>${h(j.name)}</dd><dt>Tipo</dt><dd>${KIND[j.kind] || j.kind}</dd>
      <dt>Estado</dt><dd><span class="badge ${j.state}">${STATE[j.state] || j.state}</span>${scanBadge(j)}${canRescan(j) ? ' <a href="#" id="rescan" class="muted" title="Volver a analizar los archivos guardados">↻ analizar</a>' : ''}${j.retryable ? ' <a href="#" id="retry" title="Volver a lanzar con los mismos parámetros">↻ reintentar</a>' : ''} <span class="muted">${h(j.message)}</span></dd>
...
function androidEvent(kind, payload) {
  ...
  if (kind === 'folder') { toast(payload ? `Carpeta de descargas: ${payload}` : 'Se usará Descargas/uni-share', 'ok'); const f = $('#s-folder'); if (f) f.textContent = payload || 'Descargas/uni-share'; return; }
...
function openNew(mode = 'lan', preset = {}) {
  ...
  const previewTicket = async (v) => { const pv = $('#f-prev', m); v = v.trim(); if (!pv) return; if (!/^unishare:|\.unishare$|^\{/.test(v)) { pv.hidden = true; return; }
    try { const d = await api('/api/ticket/parse', { method: 'POST', body: { data: v } }); pv.hidden = false; pv.innerHTML = `<div class="sum">🎫 ${h(d.summary)}${d.expired ? ' <span style="color:var(--err)">(expirado)</span>' : ''}</div>` + (d.signature && d.signature.status === 'invalid' ? `<div style="color:var(--err);font-weight:600">✗ FIRMA INVÁLIDA: ${h(d.signature.reason)} — ticket manipulado o falsificado</div>` : d.signature && d.signature.status === 'valid' ? `<div style="color:var(--ok)">✓ Firma Ed25519 válida · firmante ${h(d.signature.fingerprint)}</div>` : '') + d.ticket.sources.map(s => s.kind === 'lan' ? `[LAN] ${h(s.name || 'receptor')} · ${h(s.host)}:${s.port}${s.pin ? ' · PIN incluido' : ''}` : `[${s.kind}] ${h(s.url)}${s.password ? ' 🔒' : ''}`).join('<br>') + (d.ticket.files.length ? '<br>' + d.ticket.files.slice(0, 100).map(f => `${h(f.path)} <span class="muted">(${fmtB(f.size)})${f.blake3 ? ' ✓' : ''}</span>`).join('<br>') : ''); } catch (e) { pv.hidden = false; pv.innerHTML = `<span style="color:var(--err)">${h(e.message)}</span>`; } };
...
function retryJob(j) {
  if (!j || !j.retryable) return;
  api(`/api/jobs/${j.id}/retry`, { method: 'POST' }).then(r => { S.sel = r.id; toast('Reintentando…', 'ok'); poll(); }).catch(er => toast(er.message, 'err'));
}
