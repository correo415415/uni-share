...
- [x] Pendientes menores de la GUI móvil (de la etapa 6): *pull to refresh* en Dispositivos, deslizar filas para cancelar/eliminar, preparación i18n (tabla `T{}` de cadenas en `m.js`), capturas 412×915 en CI.

## Fase 13 — Limpieza, SwissTransfer con contraseña, YARA, Android Downloads/unishare
Peticiones de la sesión 09-2026 (segunda tanda tras probar la app).

- [x] **Quitar Smash.** Eliminados `src/global/smash.rs`, `upload_entries_smash`, `--backend smash`, `global.smash_api_key`/`smash_region` de `config.toml`, `crc32fast`, y todas las menciones en README/ayuda. `global.backend` solo admite `storage_to`.
- [x] **SwissTransfer: descarga de links con contraseña.** Flujo verificado en vivo (09-09-2026) con `https://www.swisstransfer.com/dl/01a086f1-7dcf-71e9-b8b5-6c1dc79c8e6e` (contraseña `test1234`): `GET /dl/{id}` → `component: link/password` → `POST /dl/{id}` `{password}` con `X-Inertia`, `X-Inertia-Version` y `X-XSRF-TOKEN` → `200 JSON` con `link/show` + `transfer.files[]` (contraseña errónea: `302` → página `link/password` con `errors.password = "Contraseña incorrecta"`) → `GET /api/1/links/{id}/files/{fid}` con la cookie `ST_SESSION` → URL S3 pre-firmada (sin sesión: `403 access_denied`). Errores tipados `PasswordRequired` / `WrongPassword`; la CLI pregunta la contraseña de forma interactiva (3 intentos) si no se pasa `--password`; el engine devuelve mensajes claros a la GUI («requiere contraseña» / «contraseña rechazada»). Tests unitarios con la respuesta capturada + test `#[ignore]` en vivo (`cargo test --lib swisstransfer -- --ignored`, variables `UNI_SHARE_ST_LINK`/`UNI_SHARE_ST_PASSWORD`).
- [x] **Android: la lista no se actualiza al crear una transferencia** hasta recargar. Causa: `/api/send-lan|send-global|download` y `offers/{id}/accept` devolvían `{job: <número>}` y `m.js` leía `r.job.id` (→ `undefined`), así que no abría el detalle ni fusionaba nada, y el SSE del WebView puede quedarse dormido mientras se anima la hoja. Arreglo: los endpoints devuelven el `Job` completo (`{job, job_id}`), `jobCreated()` lo inserta en el snapshot al instante, abre su ficha y re-pide `/api/state` por si el stream está parado.
