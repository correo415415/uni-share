...
- [ ] Etapa 4b — permiso `POST_NOTIFICATIONS` explicado (diálogo previo), notificación de oferta entrante con acciones Aceptar/Rechazar.
- [ ] **Etapa 6 — GUI móvil propia desde cero** (sustituye al WebView de escritorio encogido; misma identidad «Graphite»: superficies, acento azul único, ámbar saliente, radios 4/5/8, Inter/JetBrains Mono, badges rectangulares, sin degradados). Nivel «super profesional», pensada para pulgar y pantallas 360-430 px, modo claro/oscuro del sistema, *safe areas*, animaciones sobrias de 150 ms.
  - Decisión técnica pendiente: (a) `Slint` con `backend-android-activity-06` (misma tecnología que el escritorio, un solo lenguaje, `ui/mobile/*.slint`) o (b) capa web móvil separada (`src/gui/mobile/*`) servida por el mismo engine y cargada en el WebView. Criterios: rendimiento de listas, teclado/IME, SAF/cámara (necesitan puente Kotlin en ambos casos), tiempo de compilación en CI.
  - Apartados (barra inferior de 5 pestañas + FAB «Nueva»): **Inicio** (estado del dispositivo, actividad reciente, accesos rápidos Enviar/Recibir/Escanear QR), **Transferencias** (lista con filtros por chips, progreso, deslizar para cancelar/eliminar, ficha de detalle con Archivos/Compartir/Registro/Seguridad), **Dispositivos LAN** (radar de descubrimiento, emparejar con PIN/QR, confianza por huella), **Compartir** (ticket `.unishare` + QR grande a pantalla completa, link global, copiar/compartir con la hoja del sistema), **Ajustes** (nombre, carpeta SAF, PIN, auto-aceptar, notificaciones, escáner, límite de velocidad, caducidad, huellas TLS/Ed25519, «Acerca de»).
  - Flujos móviles: hoja inferior «Nueva transferencia» (Enviar LAN · Compartir link · Descargar · Ticket) con selector de archivos del sistema (`ACTION_OPEN_DOCUMENT` multi) y de carpeta SAF; ofertas entrantes como tarjeta destacada + notificación con acciones Aceptar/Rechazar; escáner QR integrado (zxing) para tickets y emparejamiento; estados vacíos ilustrados con el glifo de marca; *pull to refresh* en dispositivos; *haptics* discretos.
  - Calidad: capturas en CI con emulador headless (o `slint-viewer` para la variante Slint) en 360×800 y 412×915, claro y oscuro; accesibilidad (TalkBack labels, contraste AA, objetivos ≥ 48 dp); textos en español con i18n preparado (`gettext`/`slint-tr`).
