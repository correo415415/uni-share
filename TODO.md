...
- [ ] Etapa 4 — pulido móvil: `WifiManager.createMulticastLock` para mDNS en algunos fabricantes; hoja de ajustes «Carpeta de descargas» vía SAF (`ACTION_OPEN_DOCUMENT_TREE`); escáner QR (`zxing`) para tickets/emparejamiento; permiso `POST_NOTIFICATIONS` explicado; compartir hacia fuera con `FileProvider` (ya declarado).
- [x] Etapa 5 — CI: paso «Android» en `build.yml` que solo actúa si el runner tiene SDK+NDK (`ANDROID_HOME`/`ANDROID_NDK_HOME` o `~/Android/Sdk`): instala `cargo-ndk` y los targets, ejecuta `android/build-android.sh --apk` y publica `uni-share-<ver>-android-debug.apk` en la release nightly; si no hay SDK se omite con un aviso.
